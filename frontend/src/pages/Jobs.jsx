...
export default function Jobs() {
    ...
    const clearFilters = () => { const c = { search: '', city: '', district: '', recent_days: '' }; setFilters(c); fetchJobs(c); };
        ...
        <div className="py-16 pt-28">
            <div className="max-w-[1280px] mx-auto px-6">
                ...
                <form onSubmit={handleSearch} className="mb-10 animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
                    <div className="flex items-center gap-3 bg-white border border-border-subtle rounded-2xl px-5 py-3 shadow-sm focus-within:border-accent-primary focus-within:shadow-[0_0_0_3px_rgba(37,99,235,0.1)]">
                        <Search size={20} className="text-text-muted shrink-0" />
                        <input type="text" placeholder="Search by position or description..." value={filters.search} onChange={(e) => setFilters({ ...filters, search: e.target.value })} className="flex-1 bg-transparent text-text-primary text-sm placeholder:text-text-muted outline-none" />
                        <Button type="submit" size="sm">Search</Button>
                        <button type="button" className="text-text-muted hover:text-accent-primary transition-colors p-2 rounded-lg hover:bg-accent-soft" onClick={() => setShowFilters(!showFilters)}><Filter size={18} /></button>
                    </div>
                    {showFilters && (
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4 p-5 bg-white border border-border-subtle rounded-2xl shadow-sm animate-fade-in">
                ...
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-20 gap-4 text-text-secondary"><div className="spinner-lg" /><p>Finding jobs for you...</p></div>
                ) : jobs.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-20 gap-4 text-text-muted animate-fade-in"><Briefcase size={48} /><h3>No jobs found</h3><p className="text-sm">Try adjusting your search or filters</p></div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {jobs.map((job, i) => (
                            <Card key={job.id || i} className="flex flex-col animate-fade-in-up" style={{ animationDelay: `${i * 0.05}s` }}>
                                ...
                                <p className="text-sm text-text-secondary mb-4 flex-1">{job.description ? job.description.slice(0, 120) + (job.description.length > 120 ? '...' : '') : 'No description'}</p>
                                ...
                                <div className="pt-4 border-t border-border-subtle">
                                    <Link to={`/jobs/${job.id}`}><Button variant="secondary" size="sm" fullWidth>View Details</Button></Link>
                                </div>
                            </Card>
                        ))}
                    </div>
